...
//******************************************************************
  ...
  for (i = 1; i<= boxes; i++)
    ...
    if ( i > defs )   // no b.c. specification for box i:
      ...
      if (VERBOSE) fprintf(stderr,"    Box #%d: boundary cond'ns: xmin:%s xmax:%s\n", i, bc1, bc2);
      set_bc(i-1, bc1, bc2, bc2, bc2, bc2, bc2);
      }
    else if (DIMENSIONALITY == 2)
      {
      params.trail_pars(id, "bc", i, 's', bc1, 's', bc2, 's', bc3, 's', bc4,  'E');
      if (VERBOSE) fprintf(stderr,"    Box #%d: boundary cond'ns: xmin:%s xmax:%s ymin:%s ymax:%s\n", i, bc1, bc2, bc3, bc4);
      ...
      if (VERBOSE) fprintf(stderr,"    Box #%d: boundary cond'ns: xmin:%s xmax:%s ymin:%s ymax:%s zmin:%s zmax:%s\n", i, bc1, bc2, bc3, bc4, bc5, bc6);
...
FieldObj::FieldObj(const FieldObj &f) : VectorObj(f)
{
  ...
  if (fieldObstNum) {
    ...
    for (int i=0; i < fieldObstNum; i++)
      fieldObstArray[i] = f.fieldObstArray[i];
  }
...
signed long FieldObj::location_to_index(double x, double y, double z, bool errorMsg)
{
	...
	if (errorMsg) throw makeMessage("Can't interpret location (%g, %g, %g)", x, y, z);
	return -1;
}
...
BufferArray::BufferArray(TokenString &params) {
    ...
    for (int i=0; i < params.token_count("buffer"); i++) {
	  ...
	  if ( params.equal(p, "cooperative") )  
		  params.errorMessage( p, makeMessage("Cooperative buffers not allowed: use CalC versions 7+") );
